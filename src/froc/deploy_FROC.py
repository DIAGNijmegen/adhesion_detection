...
def preprocess_softmax_static(
    softmax, threshold=0.10, min_voxels_detection=10, max_prob_round_decimals=4
):
    """
...
def preprocess_softmax_dynamic(
    ...
    while len(confidences) < num_lesions_to_extract:
        ...
        all_hard_blobs, _, _ = preprocess_softmax_static(
            working_softmax,
            threshold=threshold,
...
def preprocess_softmax(
    ...
    if threshold == "dynamic":
        all_hard_blobs, confidences, indexed_pred = preprocess_softmax_dynamic(
            softmax,
            min_voxels_detection=min_voxels_detection,
        ...
        threshold = max_prob / dynamic_threshold_factor
        all_hard_blobs, confidences, indexed_pred = preprocess_softmax_static(
            softmax, threshold=threshold, min_voxels_detection=min_voxels_detection
        )
    elif threshold == "otsu":
...
def compute_pred_vector(
    ...
    min_voxels_detection=10,
    dynamic_threshold_factor=2.5,
    dynamic_threshold_num_lesions=5,
    remove_adjacent_lesion_candidates=True,
):
    y_list = []
    ...
    if label.any():  # For Each Malignant Scan
        ...
        for lesiong_id in range(1, num_gt_lesions + 1):
            ...
            if 0 in overlapped_lesions_pred:
                overlapped_lesions_pred.remove(0)
            ...
            # Multiple Predictions
                ...
                y_pred_for_target_gt = sorted(
                    y_pred_for_target_gt, key=takeSecond, reverse=True
                )
        ...
        if 0 in remaining_lesions:
            remaining_lesions.remove(0)
        for lesion_id_pred, lesion_confidence in confidences:
        ...
        if len(confidences) > 0:
            ...
            y_list.append((0, 0))  # Avoid Empty List
...
def y_to_FROC(
    ...
    y_list_pnp,
    total_patients,
    total_normal_patients,
    threshold_mode="unique",
    num_thresholds=50,
    ...
    if threshold_mode == "unique":
        ...
        if len(thresholds) > 1000:
            ...
            thresholds = [
                t for t in thresholds if t > 0.8 or t in st or t in low_thresholds
            ]
...
def compute_FROC(
    ...
    if all_softmax is None or all_labels is None:
        ...
        counter = 0
        all_softmax = np.zeros(
            shape=(val_size, patch_dims[0], patch_dims[1], patch_dims[2]),
            dtype=np.float64,
        )
        all_labels = np.zeros(
            shape=(val_size, patch_dims[0], patch_dims[1], patch_dims[2]),
        ...
        for f in os.listdir(softmax_dir):
            if "_softmax.npy" in f:
                ...
                all_labels[counter, :, :, :] = np.load(
                    softmax_dir + f.split("_softmax")[0] + "_label.npy"
                )
                counter += 1
    else:
    ...
    FP_per_image, FP_per_normal_image, sensitivity, thresholds = y_to_FROC(
        y_list_all,
        y_list_pnp,
        total_patients,
        total_normal_patients,
...
def decision_fusion(
    lambda_penalty,
    stage2_case_threshold,
    stage2_patch_threshold,
    stage2_csv_path,
    stage1_save_path,
    dual_stage_save_path,
    stage2_roi_dims,
    stage2_num_patches,
    prior_path=None,
    display=False,
    ...
    for i in range(num_classifiers):
        prediction_csv.append(
            pd.read_csv(
                stage2_csv_path[i], dtype=object, keep_default_na=False, na_values=[]
            ).values
        )
    ...
    for scan in range(len(prediction_csv[0])):
        ...
        if (patch_counter - 1) % stage2_num_patches == 0:
            ...
            stage2_roi = stage1_softmax[
                skip_slices_z : skip_slices_z + stage2_roi_dims[0],
                skip_slices_x : skip_slices_x + stage2_roi_dims[1],
                skip_slices_y : skip_slices_y + stage2_roi_dims[2],
            ].copy()
            ...
            if prior_path != None:
                ...
                prior = np.zeros_like(stage1_softmax)
            prior_roi = prior[
                skip_slices_z : skip_slices_z + stage2_roi_dims[0],
                skip_slices_x : skip_slices_x + stage2_roi_dims[1],
                skip_slices_y : skip_slices_y + stage2_roi_dims[2],
            ...
            mask[
                mask > 0.0010
            ] = 1  # Retain Predictions Within 1% Likelihood of the Presence of Prostate
        ...
        if patch_counter % stage2_num_patches == 0:
            ...
            if np.min(stage2_class0_probs) > stage2_case_threshold:
                if stage2_class0_probs[0] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[0:8, 0:64, 0:64] = (
                        stage2_roi[0:8, 0:64, 0:64]
                        * lambda_penalty
                        * prior_roi[0:8, 0:64, 0:64]
                    )
                    stage1_softmax[0:11, 0:80, 0:80] = (
                        stage1_softmax[0:11, 0:80, 0:80]
                        * lambda_penalty
                        * prior[0:11, 0:80, 0:80]
                    )

                if stage2_class0_probs[1] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[4:12, 0:64, 0:64] = (
                        stage2_roi[4:12, 0:64, 0:64]
                        * lambda_penalty
                        * prior_roi[4:12, 0:64, 0:64]
                    )
                    stage1_softmax[7:18, 0:80, 0:80] = (
                        stage1_softmax[7:18, 0:80, 0:80]
                        * lambda_penalty
                        * prior[7:18, 0:80, 0:80]
                    )

                if stage2_class0_probs[2] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[0:8, 48:112, 0:64] = (
                        stage2_roi[0:8, 48:112, 0:64]
                        * lambda_penalty
                        * prior_roi[0:8, 48:112, 0:64]
                    )
                    stage1_softmax[0:11, 64:144, 0:80] = (
                        stage1_softmax[0:11, 64:144, 0:80]
                        * lambda_penalty
                        * prior[0:11, 64:144, 0:80]
                    )

                if stage2_class0_probs[3] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[4:12, 48:112, 0:64] = (
                        stage2_roi[4:12, 48:112, 0:64]
                        * lambda_penalty
                        * prior_roi[4:12, 48:112, 0:64]
                    )
                    stage1_softmax[7:18, 64:144, 0:80] = (
                        stage1_softmax[7:18, 64:144, 0:80]
                        * lambda_penalty
                        * prior[7:18, 64:144, 0:80]
                    )

                if stage2_class0_probs[4] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[0:8, 0:64, 48:112] = (
                        stage2_roi[0:8, 0:64, 48:112]
                        * lambda_penalty
                        * prior_roi[0:8, 0:64, 48:112]
                    )
                    stage1_softmax[0:11, 0:80, 64:144] = (
                        stage1_softmax[0:11, 0:80, 64:144]
                        * lambda_penalty
                        * prior[0:11, 0:80, 64:144]
                    )

                if stage2_class0_probs[5] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[4:12, 0:64, 48:112] = (
                        stage2_roi[4:12, 0:64, 48:112]
                        * lambda_penalty
                        * prior_roi[4:12, 0:64, 48:112]
                    )
                    stage1_softmax[7:18, 0:80, 64:144] = (
                        stage1_softmax[7:18, 0:80, 64:144]
                        * lambda_penalty
                        * prior[7:18, 0:80, 64:144]
                    )

                if stage2_class0_probs[6] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[0:8, 48:112, 48:112] = (
                        stage2_roi[0:8, 48:112, 48:112]
                        * lambda_penalty
                        * prior_roi[0:8, 48:112, 48:112]
                    )
                    stage1_softmax[0:11, 64:144, 64:144] = (
                        stage1_softmax[0:11, 64:144, 64:144]
                        * lambda_penalty
                        * prior[0:11, 64:144, 64:144]
                    )

                if stage2_class0_probs[7] > stage2_patch_threshold:
                    print(subject_id)
                    stage2_roi[4:12, 48:112, 48:112] = (
                        stage2_roi[4:12, 48:112, 48:112]
                        * lambda_penalty
                        * prior_roi[4:12, 48:112, 48:112]
                    )
                    stage1_softmax[7:18, 64:144, 64:144] = (
                        stage1_softmax[7:18, 64:144, 64:144]
                        * lambda_penalty
                        * prior[7:18, 64:144, 64:144]
                    )
...
def compute_pred_stats(softmax_dir, patch_dims, val_size, threshold=0.50):
    ...
    counter = 0
    all_softmax = np.zeros(
        shape=(val_size, patch_dims[0], patch_dims[1], patch_dims[2]), dtype=np.float64
    )
    all_labels = np.zeros(
        shape=(val_size, patch_dims[0], patch_dims[1], patch_dims[2]), dtype=np.uint8
    )
    all_subject_ids = []
    for f in os.listdir(softmax_dir):
        if "_softmax.npy" in f:
    ...
    for f in range(all_softmax.shape[0]):
        ...
        if math.isnan(sensitivity[0]):
            benign_list.append(subject_id)  # Benign Scans (Redundancy Check)
        elif sensitivity[0] == 0.0:
            FN_list.append(subject_id)  # False Negatives (Missed the Lesion)
        elif sensitivity[0] == 1.0:
            fTP_list.append(subject_id)  # All True Positives (Caught Everything)
        elif (sensitivity[0] > 0.0) & (sensitivity[0] < 1.0):
            pTP_list.append(
                subject_id
            )  # Partial True Positives (Caught A Few, Missed A Few)
